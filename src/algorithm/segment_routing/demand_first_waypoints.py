...
class DemandsFirstWaypoints(GenericSR):
    ...
    def __demands_first_waypoints(self):
        ...
        overload_objective = 100*sum({(i, j): max([flow_map[(i, j)] - self.__capacity_map[i][j], 0])
                                      for i, j in self.__links}.values()) / sum({(i, j): self.__capacity_map[i][j]
                                                                                 for i, j in self.__links}.values())
        return self.__loads, waypoints, best_objective, overload_objective
    ...
    def solve(self) -> dict:
        ...
        loads, waypoints, objective, overload_objective = self.__demands_first_waypoints()
