...
class WaypointMultipathOptimized(GenericSR):
    BIG_M = 10 ** 9
    ...
    def __get_ideal_split(self, current_util_map, new_util_map):
        ...
        # main part, where we find intersections and approach/reach the global minimum mlu
    ...
    def __waypoint_multipath_optimized(self):
        """ main procedure """
        ...
        for d_map_idx in range(len(self.__demands)):
            ...
            for waypoint in range(self.__n):
                if waypoint == s or waypoint == t:
                    continue
                test_flow_map = self.__update_flow_map(sp_fraction_map, best_flow_map, s, t, d, waypoint)
                test_util_map, test_objective = self.__compute_utilization(test_flow_map)

                split = self.__get_ideal_split(best_util_map, test_util_map)

                if split["objective"] < current_best_objective:
                    split_flow_map = self.__update_flow_map(sp_fraction_map, current_best_flow_map, s, t, d, waypoint, split["split"])
                    split_util_map, split_objective = self.__compute_utilization(split_flow_map)
                    if split_objective < current_best_objective:  # safety check
                        current_best_flow_map = split_flow_map
                        current_best_util_map = split_util_map
                        current_best_objective = split_objective
                        current_best_waypoint = waypoint
                        current_best_split = split["split"]

            if current_best_waypoint is not None:
                waypoints[d_idx] = ([(s, current_best_waypoint, t)], current_best_split)
            else:
                waypoints[d_idx] = ([(s, t)], current_best_split)
    ...
    def solve(self) -> dict:
        ...
        loads, waypoints, objective = self.__waypoint_multipath_optimized()
    ...
    def get_name(self):
        ...
        return f"waypoint_multipath_optimized"
