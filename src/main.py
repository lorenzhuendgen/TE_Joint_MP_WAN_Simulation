...
def abilene_prio_algorithms():
    """ Sets up tests for topology Abilene (snd_lib) and synthetic demands using MCF MAXIMAL.
    Each test instance is executed on all prio algorithms
    """
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_prio_algorithms.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def abilene_waypoint_algorithms():
    """ Sets up tests for topology Abilene (snd_lib) and synthetic demands using MCF MAXIMAL.
    Each test instance is executed on all waypoint algorithms
    """
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_waypoint_algorithms.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def abilene_overload():
    """ Sets up tests for topology Abilene (snd_lib) and synthetic demands using MCF MAXIMAL.
    Each test instance is executed on all available algorithms """

    # algorithm settings
    algorithms = [  # ("algorithm_name", "ilp_method")
        ("waypoint_congestion_control", ""),
        ("demand_first_waypoints", ""),
        #("heur_ospf_weights", ""),
        ("inverse_capacity", ""),
        #("sequential_combination", ""),
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_overload.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def all_topologies_synthetic_demands_overload():
    """ Sets up tests using all topology data having complete link capacity data from SNDLib and TopologyZoo.
    For these tests synthetic demands generated with MCF MAXIMAL are used.
    Each test instance is executed on 4 heuristic algorithms """
    ...
    result_filename = os.path.join(OUT_DIR, "results_all_topologies_overload.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def snd_real_demands_overload():
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_real_demands_overload.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def abilene_prio_vp():
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_priority_vp.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def abilene_nanonet_algorithms():
    """ Sets up tests for topology Abilene (snd_lib) and synthetic demands using MCF MAXIMAL.
    Each test instance is executed on all waypoint algorithms
    """
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_nanonet_algorithms.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
    ...
    for links, n, topology in topology_generator:
        # setup topology specific demand generator and iterate over 10 samples of demands
        demands_generator = get_demands_generator_nanonet(topology)
        for demands, demands_provider, sample_idx in demands_generator:
            # perform each test instance on each algorithm
            for algorithm, ilp_method in algorithms:
                setup = get_setup_dict(algorithm, demands, demands_provider, links, ilp_method, n, sample_idx, test_idx,
                                       topology, topology_provider, ACTIVE_PAIRS_FRACTION, "", SEED)

                print(f"submit test: {test_idx} ({topology}, {algorithm} {ilp_method}, D_idx = {sample_idx})")
                success, objective, results = work_nanonet(algorithm, links.copy(), n, demands.copy(), ilp_method, setup,
                                          ALGORITHM_TIME_OUT, result_handler)
                print(f"Test-ID: {test_idx}, success: {success} [{algorithm} {ilp_method}, "
                      f"{topology}, {sample_idx}]: objective: {round(objective, 4)}")

                print(results['waypoints'])
...
def main():
    ...
    # print(f"Start {HIGHLIGHT}MCF Synthetic Demands - Prio Algorithms - Abilene{CEND}:")
    # abilene_prio_algorithms()

    # Evaluation waypoint algorithms
    #print(f"Start {HIGHLIGHT}MCF Synthetic Demands - Waypoint Algorithms - Abilene{CEND}:")
    #abilene_waypoint_algorithms()

    # Evaluation overload algorithms Abilene
    # print(f"Start {HIGHLIGHT}MCF Synthetic Demands - Overload - Abilene{CEND}:")
    # abilene_overload()

    # Evaluation overload algorithms Many Topologies
    print(f"Start {HIGHLIGHT}MCF Synthetic Demands - Overload - Many Topologies{CEND}:")
    all_topologies_synthetic_demands_overload()

    # Evaluation overload algorithms Real Demands
    # print(f"Start {HIGHLIGHT}Scaled Real Demands - Overload - Abilene, Geant{CEND}:")
    # snd_real_demands_overload()

    # Evaluation priority_vp algorithms
    # print(f"Start {HIGHLIGHT}MCF Synthetic Demands - prio_vp - Abilene{CEND}:")
    # abilene_prio_vp()

    # Evaluation nanonet algorithms
    #print(f"Start {HIGHLIGHT}JSON Demands - Nanonet Algorithms - Nanonet{CEND}:")
    #abilene_nanonet_algorithms()
